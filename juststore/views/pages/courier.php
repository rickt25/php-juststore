...
<?php
  ...
  $data = $courier->getCourier();

  $error = isset($_SESSION['error']['courier']) ? $_SESSION['error']['courier'] : '';
...
<main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4"><div class="chartjs-size-monitor" style="position: absolute; left: 0px; top: 0px; right: 0px; bottom: 0px; overflow: hidden; pointer-events: none; visibility: hidden; z-index: -1;"><div class="chartjs-size-monitor-expand" style="position:absolute;left:0;top:0;right:0;bottom:0;overflow:hidden;pointer-events:none;visibility:hidden;z-index:-1;"><div style="position:absolute;width:1000000px;height:1000000px;left:0;top:0"></div></div><div class="chartjs-size-monitor-shrink" style="position:absolute;left:0;top:0;right:0;bottom:0;overflow:hidden;pointer-events:none;visibility:hidden;z-index:-1;"><div style="position:absolute;width:200%;height:200%;left:0; top:0"></div></div></div>
      ...
      <div class="row mb-2 justify-content-between">
            <div class="col-md-5">
                <button class="btn btn-primary" id="btn-addcourier" data-toggle="modal" data-target="#addcourier">+ Add Courier</button>
            </div>
            <div class="col-md-4">
            ...
            <tbody>
              ...
              ?>
                <tr>
                    ...
                    <td>Rp. <?php echo $d['cost'] ?></td>
                    <td class="courier-custom-table"><img src="assets/courier/<?php echo $d['icon'] ?>" class="courier-img-table" alt=""><?php echo $d['icon'] ?></td>
                    <td>
                        <form action="user/deleteCourier.php" method="POST">
                          <a class="btn btn-info" href="" data-toggle="modal" data-target="#editCourier<?php echo $d['id'] ?>">
                ...
                <div class="modal fade" id="editCourier<?php echo $d['id'] ?>" tabindex="-1" aria-labelledby="editCourierModal" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered"">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="">Edit Courier</h5>
                          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                          </button>
                        </div>
                        <div class="modal-body">
                          <form action="user/editCourier.php" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                              ...
                              <input type="number" min="0" name="cost" class="form-control <?php echo isset($error['cost']) ? 'is-invalid' : '' ?>" value="<?php echo $d['cost'] ?>" placeholder="Enter cost" id="cost" aria-describedby="name">
                              <?php if(isset($error['cost'])){ ?>
                                <div class="invalid-feedback"><?php echo $error['cost']; ?></div>
                              <?php } ?>
                            </div>
...
<div class="modal fade" id="addcourier" tabindex="-1" aria-labelledby="addUserModal" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered"">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="">Add Courier</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="user/addCourier.php" method="POST" enctype="multipart/form-data">
          <div class="form-group">
            <label for="name">Name</label>
          ...
          <div class="form-group">
            <label for="cost">Cost</label>
            <input type="number" min="0" class="form-control <?php echo isset($error['cost']) ? 'is-invalid' : '' ?>" placeholder="Input cost" name="cost" id="cost"></textarea>
            <?php if(isset($error['cost'])){ ?>
              <div class="invalid-feedback d-block"><?php echo $error['cost']; ?></div>
            <?php } ?>
          </div>
